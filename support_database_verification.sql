-- =====================================================
-- IMPROMPTUINDIAN SUPPORT DATABASE VERIFICATION
-- =====================================================
...
-- Check all flows by order status
SELECT 
    order_status,
    COUNT(*) as flow_count,
    GROUP_CONCAT(
        CONCAT(issue_key, ' (', issue_title, ')') 
        SEPARATOR ' | '
    ) as available_flows
...
-- Check coverage: Which order statuses have flows?
SELECT 
    sof.order_status,
    COUNT(*) as flow_count,
    COUNT(CASE WHEN sof.ai_reply IS NOT NULL AND sof.ai_reply != '' THEN 1 END) as flows_with_replies,
    COUNT(CASE WHEN sof.auto_resolve = 1 THEN 1 END) as auto_resolve_flows,
    COUNT(CASE WHEN sof.escalate_if_selected = 1 THEN 1 END) as escalate_flows
FROM impromptuindian_admin.support_order_flows sof
GROUP BY sof.order_status
ORDER BY sof.order_status;

-- =====================================================
-- 2. VERIFY THREADS TABLE (Message Storage)
-- =====================================================

-- Overall threads statistics
SELECT 
...
FROM impromptuindian_admin.threads th
JOIN impromptuindian_admin.support_tickets t ON t.id = th.ticket_id
...
FROM impromptuindian_admin.support_tickets t
...
SELECT 
    status,
    COUNT(*) as count,
    ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM impromptuindian_admin.support_tickets), 2) as percentage,
    AVG(TIMESTAMPDIFF(HOUR, created_at, COALESCE(resolved_at, NOW()))) as avg_hours_open
...
FROM impromptuindian_admin.support_users su
LEFT JOIN impromptuindian_admin.support_tickets st ON st.assigned_agent_id = su.id
WHERE su.role = 'agent'
...
SELECT * FROM impromptuindian_admin.v_agent_performance
...
SELECT * FROM impromptuindian_admin.v_support_health;
...
SELECT * FROM impromptuindian_admin.v_vendor_issues
...
SELECT 
    ...
    COUNT(*) as count
FROM impromptuindian_admin.threads th
LEFT JOIN impromptuindian_admin.support_tickets t ON t.id = th.ticket_id
WHERE t.id IS NULL;
...
SELECT 
    ...
    COUNT(*) as count
FROM impromptuindian_admin.support_tickets t
JOIN impromptuindian_admin.threads th ON th.ticket_id = t.id
...
FROM impromptuindian_admin.support_users su
JOIN impromptuindian_admin.support_tickets st ON st.assigned_agent_id = su.id
...
-- 
-- Expected Results:
-- 1. support_order_flows should have flows for all active order statuses
-- 2. threads table should be actively receiving messages
-- 3. All essential tables should have data
-- 4. Views should return data without errors
-- 5. No orphaned records should exist
-- =====================================================

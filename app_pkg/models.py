...
class VendorUser(db.Model):
    ...
    order_categories = db.Column(MySQLJSON, default=list)  # JSON array of order categories: ['new_orders', 'in_production', 'ready_for_dispatch', 'completed_orders']
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class Rider(db.Model):
    ...
    latitude = db.Column(db.Float)
    longitude = db.Column(db.Float)
    location_accuracy = db.Column(db.Float)  # Meters - for dispatch: ORDER BY location_accuracy ASC
...
# DEPRECATED: This model is no longer used. Use SupportUser instead.
# Kept for backward compatibility only. All new code should use SupportUser.
# class Support(db.Model):
#     __bind_key__ = 'support'
#     """Support staff model for support database (DEPRECATED - use SupportUser)"""
